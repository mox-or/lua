...
if not isfile("uzu01.png") then
    writefile("uzu01.png", game:HttpGet("https://raw.githubusercontent.com/uzu01/uzu/refs/heads/main/uzu01.png"))
end
...
function Library:AddIconToggle(Func)
	local Table = {}
	local Icon = Library:GetIcon(getcustomasset("uzu01.png"))
	...
	local ClickIn = TweenService:Create(Button, TweenInfo.new(0.08), {
		Size = UDim2.fromOffset(40, 40),
	})

	local ClickOut = TweenService:Create(Button, TweenInfo.new(0.08), {
		Size = UDim2.fromOffset(44, 44),
	})
	...
	function Table:SetDraggable(Bool)
		Library:MakeDraggable(Button, Button, Bool)
	end
...
do
	...
	function Funcs:AddViewport(...)
		...
		if typeof(First) == "table" or typeof(Second) == "table" then
			...
			Data.Size = Params.Size or UDim2.new(1, 0, 0, 200)
			Data.CameraCFrame = Params.CameraCFrame or CFrame.fromEulerAnglesYXZ(math.rad(0), math.rad(0), math.rad(0)) * CFrame.new(0, 0, 40)
			Data.CameraFOV = Params.CameraFOV or 14
			Data.Model = Params.Model or workspace:FindFirstChildWhichIsA("BasePart")
			Data.ModelCFrame = Params.ModelCFrame or CFrame.new(0, 0, 0) * CFrame.fromEulerAnglesYXZ(math.rad(1), math.rad(132), math.rad(0))
		else
			Data.Size = UDim2.new(1, 0, 0, 200)
			Data.CameraCFrame = CFrame.new(0, 0, 0)
			Data.CameraFOV = 14
			Data.Model = nil
			Data.ModelCFrame = CFrame.new(0, 0, 0)
		end
		...
		local Camera = New("Camera", {
			...
			FieldOfView = Data.CameraFOV,
			Parent = ViewportFrame,
		})
		
		local Model = Data.Model:Clone()
		Model:PivotTo(Data.ModelCFrame)
		
		ViewportFrame.CurrentCamera = Camera
		Model.Parent = ViewportFrame
		
		Groupbox:Resize()
		table.insert(Groupbox.Elements, ViewportFrame)
	...
	function Funcs:AddLabel(...)
		...
		local TextLabel = New("TextLabel", {
			...
			TextXAlignment = (Groupbox.IsKeyTab or Data.Center and Enum.TextXAlignment.Center) or Enum.TextXAlignment.Left,
			Parent = Container,
		})
...
function Library:CreateWindow(WindowInfo)
	...
	if WindowInfo.Color then
		Library.Scheme.AccentColor = WindowInfo.Color
	end
	...
	function Window:AddTab(Name: string, Icon)
		...
		function Tab:AddTabbox(Info)
			...
			local function UpdateArrow()
				...
				local icon = ActiveTab.Collapsed and DownIcon or UpIcon
				if icon then
					ArrowImage.Image = icon.Url
					ArrowImage.ImageRectOffset = icon.ImageRectOffset
					ArrowImage.ImageRectSize = icon.ImageRectSize
				end
			end
			Tabbox.UpdateArrow = UpdateArrow

			ToggleButton.MouseButton1Click:Connect(function()
				local ActiveTab = Tabbox.ActiveTab
				if not ActiveTab then return end

				ActiveTab.Collapsed = not ActiveTab.Collapsed
				ActiveTab.Container.Visible = not ActiveTab.Collapsed
				UpdateArrow()
				ActiveTab:Resize()
			end)

			local Count = -1
			function Tabbox:AddTab(Name, Icon, Title, Show)
				if Show == nil then
					Show = true
				end
				local iconData = Library:GetIcon(Icon) or Library:GetIcon("layout-grid")
				...
				local ButtonText = New("TextLabel", {
					BackgroundTransparency = 1,
					Text = (Title and Title ~= "") and Title or Name,
					TextSize = 14,
					TextTransparency = 0.5,
					Size = UDim2.fromScale(0, 1),
	...
	if Library.IsMobile then
		local ToggleButton = Library:AddIconToggle(function()
			Library:Toggle()
		end)
	end
